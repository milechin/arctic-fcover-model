...
end_year = sys.argv[3]
model = '_'+str(sys.argv[4]) if len(sys.argv) > 4 else ''
...
for year in range(int(start_year), int(end_year)+1):
    print(year)
    if os.path.exists(model_outdir+str(year)+'/FCover_'+tile+'_'+str(year)+'_rfr'+model+'.tif'):
        print(model_outdir+str(year)+'/FCover_'+tile+'_'+str(year)+'_rfr'+model+'.tif exists!')
        continue
    ################################### STEP 1 ###################################
    ...
    # Read in raster
    full_stack = rasterio.open(outdir+str(year)+'/'+tile+'/feats_full_'+str(year)+'.tif','r')
    ...
    # Phenology masking
    try:
        with rasterio.open(outdir+str(year)+'/'+tile+'/'+tile+'_'+str(year)+'_pheno_final.tif','r') as src0:
    ...
    with rasterio.open(model_outdir+str(year)+'/FCover_'+tile+'_'+str(year)+'_rfr'+model+'.tif', 'w', **meta2) as dst:
