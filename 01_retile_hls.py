...
# # For each HLS tile
# subdirs = glob.glob(outdir+'[0 1]*')
# #subdirs = glob.glob(outdir+'1*')  # For just tiles that start 1#XXX
# subdirs = [os.path.basename(subdir.split("/")[7]) for subdir in subdirs]

# OR For each AK HLS tile
convert_list = gpd.read_file('/projectnb/modislc/users/seamorez/HLS_FCover/HLS_tiles/NAA/NAA_convert.shp')
# ak_tundra_all = convert_list[convert_list['UID'].isin([3,6,7,8,39,40,41,42,43,44,45,74,75,76,77,78,79,80,81,82,109,110,111,112,113,
#                                                        115,116,117,118,146,147,148,152,153,154,155,182,183,184,190,191,217,218,219,
#                                                        220,226,227,1050,1051,1057,1058,1065,1073,1074,1081,1082,1083,1090])]
ak_tundra_all = convert_list[convert_list['UID'].isin([192,193,194,228,229,230,231,265,266,267,268,269,301,302,303,304,305,339,340,
                                                       341,375,376,377])]
subdirs = ak_tundra_all['Name'].unique()
...
# Retile imagery
# Get UID list
#convert_list = pd.read_csv('/projectnb/modislc/users/seamorez/HLS_FCover/HLS_tiles/AKTundra/AKTundra_convert.csv')  # Commented out to use list from STEP 1
uids = ak_tundra_all['UID'].unique()

# Retile spectral features to ABoVE grid for all years
for year in years:
    print(year)
    retile(year, ak_tundra_all, uids)

# Retile topographic features to ABoVE grid and save in 2016 directory
retile_topo(ak_tundra_all, uids)
