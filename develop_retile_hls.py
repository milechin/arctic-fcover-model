...
outdir = '/projectnb/modislc/users/seamorez/HLS_FCover/output_ndvi/'
indir = '/projectnb/modislc/users/seamorez/HLS_FCover/input/HLS30/'
start_yr = 2016
end_yr = 2023
phenometrics = ['OGI','50PCGI','OGMx','Peak','OGD','50PCGD','OGMn','EVImax','EVIamp','gupQA']
...
# For each HLS tile
subdirs = glob.glob(outdir+'0*')
subdirs = [os.path.basename(subdir.split("/")[7]) for subdir in subdirs]
for subdir in subdirs:
    print(subdir)
    # For each year
    for year in years:
        # For each phenometric: 50PCGI, Peak, 50PCGD
        file_list = ['/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[0],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[1],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[2],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[3],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[4],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[5],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[6],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[7],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[8],
                     '/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.nc:'+phenometrics[9]]
        ...
        if (os.path.isfile(outdir+subdir+'/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.tif')):
            continue
        
        # Write out new stacked raster for the phenometric
        with rasterio.open(outdir+subdir+'/phenoMetrics/MSLSP_'+subdir+'_'+str(year)+'.tif', 'w', **meta) as dst:
...
def retile(yr, convert_list, uids):
    ...
    target_profile.update(dtype='int16',nodata=32767,count=10)
    ...
    for tile in uids: # CHANGE ONCE READY TO RUN ON ALL!
        ...
        out_file = outdir+yrfold+tilefold+'/'+tilefold+'_'+str(yr)
        tifs = glob.glob(outdir+yrfold+tilefold+'/*.tif')
        print(tifs)
        ...
        for f in glob.glob(outdir+yrfold+tilefold+'/*'+str(yr)+'.tif'):
            os.remove(f)
...
# Retile imagery
# Get UID list
convert_list = pd.read_csv('/projectnb/modislc/users/seamorez/HLS_FCover/HLS_tiles/AKTundra/DEVELOP_convert.csv')
