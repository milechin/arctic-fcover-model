...
for i, row in map_list.iterrows():
    ...
    if not os.path.exists(hls_ref_path):
        ...
        continue
    make_fractions(bin_path, hls_ref_path, model_assessdir+row['map']+'_30m.tif')
...
for i, row in map_list.iterrows():
    ...
    with rio.open(model_assessdir+row['map']+'_30m_assess.tif') as src:  
        ...
        with rio.open(model_outdir+str(row['year'])+'/FCover_'+row['hls_tile']+'_'+str(row['year'])+'_rfr_peak.tif') as pred_src:   
